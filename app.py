...
- 주력 갱신 시간대: {peak_hour_str if top_spender_raw_name else '데이터 분석 중'}"""
    ...
    if selected_menu == "📊 오늘의 AI 성과 (핵심 요약)":
        ...
        if IS_DEMO_MODE:
            ...
            dummy_logs = [
                # 1. 갱신 직후 1위 달성 후 살짝 엎치락뒤치락 하는 리얼한 패턴
                {"갱신시간": (now_kst - timedelta(minutes=18)).strftime("%Y-%m-%d %H:%M:%S"), "단지명": "다산자이아이비플레이스", "매물상세": "1**동 (*4A)", "상태": "✅ 성공", "갱신 전 순위": "14위 (🔴하위권)", "갱신 후 최고순위": "🏆 최고 1위 (현재 1위)", "순위 궤적": [20, 20, 19, 20, 19], "성과 요약": "🚀 13계단 상승"},
                # 2. 갱신 후 2위 진입, 서서히 4위로 밀려나는 패턴
                {"갱신시간": (now_kst - timedelta(hours=1, minutes=45)).strftime("%Y-%m-%d %H:%M:%S"), "단지명": "다산한양수자인리버팰리스", "매물상세": "1**3동 (*4B)", "상태": "✅ 성공", "갱신 전 순위": "9위 (🟡중위권)", "갱신 후 최고순위": "🏆 최고 2위 (현재 4위)", "순위 궤적": [19, 19, 18, 15, 17, 16], "성과 요약": "🚀 7계단 상승"},
                # 3. 권외에서 단숨에 상위권 진입 후, 타사 공격으로 급격히 밀리는 롤링 패턴
                {"갱신시간": (now_kst - timedelta(hours=3, minutes=20)).strftime("%Y-%m-%d %H:%M:%S"), "단지명": "힐스테이트다산", "매물상세": "5**9동 (*4B)", "상태": "✅ 성공", "갱신 전 순위": "18위 (🔴하위권)", "갱신 후 최고순위": "🏆 최고 3위 (현재 11위)", "순위 궤적": [18, 15, 13, 11, 10, 10], "성과 요약": "🚀 15계단 상승"},
                # 4. 비교적 안정적으로 상위권을 방어하는 패턴
                {"갱신시간": (now_kst - timedelta(hours=6, minutes=5)).strftime("%Y-%m-%d %H:%M:%S"), "단지명": "다산유승한내들센트럴", "매물상세": "2**4동 (*4A)", "상태": "✅ 성공", "갱신 전 순위": "6위 (🟡중위권)", "갱신 후 최고순위": "🏆 최고 1위 (현재 2위)", "순위 궤적": [20, 20, 19, 18, 19, 19], "성과 요약": "🚀 5계단 상승"},
                # 5. 아예 페이지 밖(권외)에서 1페이지로 멱살 잡고 끌어올린 패턴
                {"갱신시간": (now_kst - timedelta(hours=11, minutes=40)).strftime("%Y-%m-%d %H:%M:%S"), "단지명": "다산e편한세상자이", "매물상세": "1**2동 (*4A)", "상태": "✅ 성공", "갱신 전 순위": "20위 밖(권외)", "갱신 후 최고순위": "🏆 최고 2위 (현재 6위)", "순위 궤적": [19, 18, 19, 15, 16, 15, 15], "성과 요약": "🚀 1페이지 진입 방어"},
                # 6. 하루 전 갱신, 1위 달성 후 시간이 지나며 자연스럽게 하위권으로 밀려난 현실적 패턴
                {"갱신시간": (now_kst - timedelta(hours=22, minutes=15)).strftime("%Y-%m-%d %H:%M:%S"), "단지명": "다산펜테리움리버테라스I", "매물상세": "7**5동 (*4A)", "상태": "✅ 성공", "갱신 전 순위": "12위 (🔴하위권)", "갱신 후 최고순위": "🏆 최고 1위 (현재 14위)", "순위 궤적": [20, 18, 15, 12, 9, 8, 7], "성과 요약": "🚀 11계단 상승"}
            ]
