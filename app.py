...
# --- [1] 비밀 장부(JSON) 로드 로직 (문법 오류 대응 강화) ---
    ...
    if os.path.exists("realtors.json"):
        with open("realtors.json", "r", encoding="utf-8") as f:
            try:
                ...
                # JSON 문법 에러 시 안내 출력
                st.error("🚨 'realtors.json' 파일에 쉼표(,)가 빠졌거나 문법이 틀렸습니다. 파일을 확인해주세요.")
                return {"a123": "더자이디엘"}
            except:
                return {"a123": "더자이디엘"}
    return {"a123": "더자이디엘"}
...
def process_data(df):
    ...
    # 🚨 [강화된 왜곡 방지] 2.5시간 공백 발생 후 1시간 이내 데이터는 모두 왜곡으로 간주
...
try:
    ...
    if not boosted_df.empty:
        ...
        ts_data = boosted_df[boosted_df['부동산명'] == top_spender_raw_name]
        if not ts_data.empty:
            avg_h = int(round(ts_data['수집일시'].dt.hour.mean()))
            peak_hour_str = f", 평균적으로 {avg_h}시 부근에 갱신이 집중됩니다."
            
    col4.metric("🔥 최대 지출 경쟁사", top_spender)
    st.markdown("---")

    # --- 탭 구성 (원본 탭 명칭 및 순서 유지, 인덱싱만 제거) ---
    ...
    with tab_report:
        ...
        if not danger_ls.empty:
            ...
            danger_detail = f"\n*(특히 {', '.join([f'{mask_text(k)}({v}건)' for k, v in top_danger_comps.items()])}에서 1위 이탈이 가장 많이 발생했습니다.)*"
        
        empty_detail = ""
        if not empty_houses.empty:
            top_empty_comps = empty_houses['단지명'].value_counts().head(2)
            empty_detail = f"\n*(추천 타겟: {', '.join([f'{mask_text(k)}({v}건)' for k, v in top_empty_comps.items()])} 위주로 우선 갱신을 추천드립니다.)*"

        masked_brief_spender = mask_text(clean_realtor_name(top_spender_raw_name), True) if top_spender_raw_name else "없음"
        brief_spender_text = f"{masked_brief_spender} ({stat_df.iloc[0]['총횟수']}회)" if not boosted_df.empty else "없음"
        
        briefing = f"""[📅 이실장 시장 동향 브리핑]\n(기간: {start_dt.strftime('%m/%d %H:%M')} ~ {end_dt.strftime('%m/%d %H:%M')})\n\n📊 시장 점유율 현황:\n대표님의 현재 단지별 랭킹은 [{rank_str if rank_str else "순위 없음"}] 입니다.\n\n🚨 방어전 필요: {len(danger_ls)}건{danger_detail}\n🎯 공격 타겟: {len(empty_houses)}건{empty_detail}\n🔥 경쟁사 위협: {brief_spender_text}{peak_hour_str}"""
        st.text_area("복사하여 활용하세요.", value=briefing, height=400)
    ...
    with tab_ms:
        ...
        with col_b:
            ...
            fig = px.bar(top10, x='총점수', y='부동산명_축약', orientation='h', title=f"{mask_text(filter_comp)} M/S Top 10", text='총점수', color_discrete_sequence=['#3182f6'])
    ...
    with tab_timing:
        ...
        if not boosted_raw.empty:
            ...
            show_boost['비고'] = show_boost['왜곡영역'].apply(lambda x: "⚠️ 수집공백 분석제외" if x else "정상")
    ...
    with tab_stat:
        ...
        if not boosted_df.empty:
            ...
            stat_df_final['평균시간대'] = stat_df_final['평균시간대'].apply(lambda x: f"{x}시")
            ...
            fig3 = px.line(hc, x='활동시간대', y='갱신건수', title="시장 전체 광고 갱신 분포 (왜곡 제거)", markers=True, color_discrete_sequence=['#3182f6'])
