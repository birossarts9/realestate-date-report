    ...
    if selected_menu == "📊 오늘의 AI 성과 (핵심 요약)":
        ...
        if IS_DEMO_MODE:
            ...
            dummy_logs = [
                ...
                {"갱신시간": (now_kst - timedelta(days=1, hours=5)).strftime("%Y-%m-%d %H:%M:%S"), "단지명": "다산유승한내들골든뷰", "동/호수": "3104동 (84m2)", "상태": "✅ 성공", "갱신 전 순위": "7위 (🟡중위권)", "갱신 후 순위": "2위 (🟢상위권)", "성과 요약": "🚀 5계단 상승"},
            ]
...
https://realestate-date-report.streamlit.app/?id={user_id}&ref={ref_id}""".replace("`", "'")
    ...
    # ==========================================================
        ...
        with act_tab1:
            ...
            if not danger_ls.empty:
                ...
                st.success("현재 모든 매물이 상위 안전권에 있습니다!")
    ...
    # ==========================================================
        ...
        st.info("💡 **개별 매물 심층 차트:** 특정 매물을 선택하면 해당 매물의 노출 생존율, 롤링 심각도, 추천 타격 시간, 그리고 랭킹 차트를 한 번에 종합 진단합니다.")
        ...
        if search_comp:
            ...
            search_bundle = c_search2.selectbox("🏠 매물 묶음 선택 (동/호수/가격)", bundle_list, key="search_bundle")
            ...
            if search_bundle:
                ...
                avg_rank = b_ranks.mean() if len(b_ranks) > 0 else 999
                
                if survival_rate >= 80: roi_status, roi_col, roi_desc = "🟢 집중 타격 가능", "#10b981", "네이버 알고리즘 우대 매물입니다. 봇으로 타격 시 최상단 고정이 확정적입니다."
                elif survival_rate >= 40: roi_status, roi_col, roi_desc = "🟡 가성비 방어 요망", "#f59e0b", "일반적인 롤링이 발생하는 매물입니다. 틈새 시간 공략이 필요합니다."
                else: roi_status, roi_col, roi_desc = "🔴 광고 효율 낮음", "#ef4444", "알고리즘 점수가 낮아 20위 밖으로 자주 밀려납니다. 집주인 인증 재등록을 권장합니다."

                # 2. 경쟁 강도 (추천 시간) 계산
                comp_count = len(bdf[bdf['수집일시'] == bdf['수집일시'].max()]['부동산명'].unique())
                freq = len(b_boosted)
                if freq >= 10: market_status = "🔥 초경쟁 (레드오션)"
                elif freq >= 5: market_status = "⚠️ 보통 (주기적 갱신)"
                elif search_bundle in my_empty['매물묶음키'].values: market_status = "🧊 방치된 빈집 (블루오션)"
                else: market_status = "💧 안정 (갱신 적음)"
                ...
                # 3. 종합 프로필 카드 UI
                st.markdown(f"""
                <div style="background-color: white; border: 1px solid #e2e8f0; border-radius: 15px; padding: 25px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom: 25px;">
                    <h3 style="color: #1e3a8a; margin-top: 0;">{mask_text(search_comp)} <span style="font-size: 20px; color: #64748b;">| {mask_text(search_bundle)}</span></h3>
                    <div style="display: flex; gap: 20px; margin-top: 15px; flex-wrap: wrap;">
                        <div style="flex: 1; min-width: 200px; background-color: #f8fafc; padding: 15px; border-radius: 10px; border-left: 5px solid {roi_col};">
                            <div style="font-size: 14px; color: #64748b; font-weight: bold;">알고리즘 생존율 (ROI)</div>
                            <div style="font-size: 24px; font-weight: 800; color: {roi_col}; margin: 5px 0;">{survival_rate:.1f}% ({roi_status})</div>
                            <div style="font-size: 12px; color: #475569;">평균 순위: {avg_rank:.1f}위 | {roi_desc}</div>
                        </div>
                        <div style="flex: 1; min-width: 200px; background-color: #f8fafc; padding: 15px; border-radius: 10px; border-left: 5px solid #3b82f6;">
                            <div style="font-size: 14px; color: #64748b; font-weight: bold;">시장 경쟁 상태</div>
                            <div style="font-size: 24px; font-weight: 800; color: #3b82f6; margin: 5px 0;">{market_status}</div>
                            <div style="font-size: 12px; color: #475569;">현재 묶인 부동산: {comp_count}곳</div>
                        </div>
                        <div style="flex: 1; min-width: 200px; background-color: #f8fafc; padding: 15px; border-radius: 10px; border-left: 5px solid #8b5cf6;">
                            <div style="font-size: 14px; color: #64748b; font-weight: bold;">AI 추천 타격 시간</div>
                            <div style="font-size: 24px; font-weight: 800; color: #8b5cf6; margin: 5px 0;">{rec_time if rec_time != "-" else "데이터 수집중"}</div>
                            <div style="font-size: 12px; color: #475569;">경쟁사 갱신 피크타임 분석 결과</div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                fig2 = px.line(t_hist, x='수집일시', y='전체순위차트용', markers=True, title=f"🌀 {mask_text(search_comp)} 상세 순위 추적 (🚀 AI 갱신 시점)", color_discrete_sequence=['#3182f6'])
                for rt in renew_times:
                    if start_dt <= rt <= end_dt:
