...
# --- 🚀 [수정] 데모 및 사용자 + 타겟 단지 매핑 로직 ---
IS_DEMO_MODE = (user_id == "demo")
realtor_data = REALTOR_MAP.get(user_id, "더자이디엘")

# JSON 포맷이 새 방식(dict)인지 구 방식(문자열)인지 자동으로 판별하여 대응
if isinstance(realtor_data, dict):
    my_realtor = realtor_data.get("name", "더자이디엘")
    my_complexes = realtor_data.get("complexes", [])
else:
    my_realtor = str(realtor_data)
    my_complexes = []
...
def mask_text(text, is_agent=False):
    ...
    if is_agent:
        if my_realtor in str(text): return display_realtor
...
try:
    ...
    start_date = col_sd.date_input("시작일", default_start_date)
    ...
    # -------------------------------------------------------------
    # [요청 기능] 타겟 단지 자동 필터링 로직 삽입 (기존 구조 보존)
    # -------------------------------------------------------------
    if my_complexes:
        t_df = t_df[t_df['단지명'].isin(my_complexes)].copy()

    if t_df.empty:
        st.error("설정한 기간 또는 해당 단지에 데이터가 없습니다.")
        st.stop()
    ...
    # [수정] 필터링된 데이터(t_df) 기준으로 콤보박스 리스트 생성
    ...
    with tab_report:
        ...
        <div class="master-strategy-board">
            <h2 style="color:#1e3a8a; margin-top:0; font-size:32px; margin-bottom:12px;">📊 오늘의 시장 브리핑</h2>
